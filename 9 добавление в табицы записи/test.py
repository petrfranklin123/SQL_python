...
#объект MySQLConnection хрнится в переменной connection
        ...
        #существует несколько вариантов заколнения таблицы
        #первый способ больше подходит для небольшого количества записей
        #следующий вариант более реальный 
        insert_movies_query = """
        ...
        with connection.cursor() as cursor:
            #Этот способ через execute
            cursor.execute(insert_movies_query)
            #commit необходим, таким образом мы завершаем операцию
            connection.commit()


        # в этом примере %s это значения переменных, которые передаются
        ...
        with connection.cursor() as cursor:
            #первым параметром идет шаблон запроса
            #вторым идет массив данных
            cursor.executemany(insert_reviewers_query, reviewers_records)
            connection.commit()


        #по такому же принципу выполняем этот запрос 
